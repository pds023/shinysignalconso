...
app_server <- function(input, output, session) {
  ...
  observe({
    req(input$highchart_stats_categories_treemap)
    req(input$highchart_stats_categories_pct)
    output$highchart_stats_categories <- renderHighchart(graph_explore(data = data_categories()[N > 5],
                                                                       input_treemap = input$highchart_stats_categories_treemap,
                                                                       input_pct = input$highchart_stats_categories_pct))
  })
  observe({
    req(input$highchart_stats_tags_treemap)
    req(input$highchart_stats_tags_pct)
    output$highchart_stats_tags <- renderHighchart(graph_explore(data = data_tags()[N > 5],
                                                                 input_treemap = input$highchart_stats_tags_treemap,
                                                                 input_pct = input$highchart_stats_tags_pct))
  })
  observe({
    req(input$highchart_stats_territoire_treemap)
    ...
    output$highchart_stats_territoire <- renderHighchart(graph_explore(data = data_territory()[N > 5,.(reg_name,dep_name,N)],
                                                                       input_treemap = input$highchart_stats_territoire_treemap,
                                                                       input_pct = input$highchart_stats_territoire_pct,
                                                                       group = TRUE))
  })
  observe({
    req(input$highchart_stats_sigstate_treemap)
    req(input$highchart_stats_sigstate_pct)
    output$highchart_stats_sigstate <- renderHighchart(graph_explore(data = data_sigstate()[N > 5],
                                                                     input_treemap = input$highchart_stats_sigstate_treemap,
                                                                     input_pct = input$highchart_stats_sigstate_pct))
  })
